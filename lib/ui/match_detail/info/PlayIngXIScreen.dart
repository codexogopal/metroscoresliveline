...
class PlayIngXIScreen extends StatefulWidget{
  ...
  final String clickedFrom;
  const PlayIngXIScreen({super.key, required this.matchId, required this.clickedFrom});
  @override
  State<StatefulWidget> createState() => PlayIngXIScreenState();
}

class PlayIngXIScreenState extends State<PlayIngXIScreen>{

  @override
  void initState() {

    Future.delayed(const Duration(milliseconds: 200)).then((_) {
      if(Provider.of<MatchDetailController>(context, listen: false).loadedMatchId == "" || Provider.of<MatchDetailController>(context, listen: false).teamData == null) {
        Provider.of<MatchDetailController>(context, listen: false).isLoadingPlayer = true;
        Provider.of<MatchDetailController>(context, listen: false).getPlayingXiByMatchId(context, widget.matchId.toString(), widget.clickedFrom);
      }
    });
    Provider.of<MatchDetailController>(context, listen: false).loadPlayerData(widget.clickedFrom);
    super.initState();
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Consumer<MatchDetailController>(builder: (context, myData, _){
      return Scaffold(
        appBar: AppBars.myAppBar("Playing XI", context, true),
        body: SafeArea(
          child: Column(
            children: [
              Expanded(
  ...
  Widget addBody(){
    MatchDetailController provider = Provider.of(context, listen: false);
    double teamWidth = 30;
    List squadsListData = provider.teamData["player"];
    return Column(
      children: [
        Visibility(
          visible: false,
          ...
          child: Row(
            ...
            children: [
              Padding(
                ...
                child: Text(provider.teamData["name"] ?? "hi", style: Theme.of(context).textTheme.titleMedium?.copyWith(),),
              ),
              InkWell(
                onTap: (){
